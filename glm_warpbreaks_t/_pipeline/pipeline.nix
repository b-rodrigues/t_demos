...
EOF
      ...
      echo "captured_warns <- list()" >> node_script.R
      echo "r_poisson <- withCallingHandlers({" >> node_script.R
      echo "  local({" >> node_script.R
      echo "    tryCatch({" >> node_script.R
      cat <<'EOF' >> node_script.R
...
EOF
      ...
      echo "  invokeRestart('muffleWarning')" >> node_script.R
      echo "})" >> node_script.R
       echo "if (r_is_error(r_poisson)) {" >> node_script.R
       echo "  r_write_error(r_poisson, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_pmml(r_poisson, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(r_poisson), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
       echo "  r_write_warnings(captured_warns, file.path(Sys.getenv('out'), 'warnings'))" >> node_script.R
       echo "}" >> node_script.R
      mkdir -p $out
      Rscript node_script.R
...
EOF
      ...
      echo "captured_warns <- list()" >> node_script.R
      echo "r_nb <- withCallingHandlers({" >> node_script.R
      echo "  local({" >> node_script.R
      echo "    tryCatch({" >> node_script.R
      cat <<'EOF' >> node_script.R
            data_node$wool <- as.factor(data_node$wool)
...
EOF
      ...
      echo "  invokeRestart('muffleWarning')" >> node_script.R
      echo "})" >> node_script.R
       echo "if (r_is_error(r_nb)) {" >> node_script.R
       echo "  r_write_error(r_nb, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_pmml(r_nb, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(r_nb), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
       echo "  r_write_warnings(captured_warns, file.path(Sys.getenv('out'), 'warnings'))" >> node_script.R
       echo "}" >> node_script.R
      mkdir -p $out
      Rscript node_script.R
  ...
  py_poisson = stdenv.mkDerivation {
    name = "py_poisson";
...
EOF
      ...
      echo "if py_is_error(py_poisson):" >> node_script.py
      echo "    py_write_error(py_poisson, os.path.join(os.environ['out'], 'artifact'))" >> node_script.py
      echo "else:" >> node_script.py
      cat <<'EOF' >> node_script.py
    py_write_pmml(py_poisson, os.path.join(os.environ['out'], 'artifact'))
EOF
      echo "    with open(os.path.join(os.environ['out'], 'class'), 'w') as f: f.write(py_visual_class(py_poisson))" >> node_script.py
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin data_node r_poisson r_nb py_poisson ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${data_node} $out/data_node
      cp -r ${r_poisson} $out/r_poisson
      cp -r ${r_nb} $out/r_nb
      cp -r ${py_poisson} $out/py_poisson
    '';
  };
}
