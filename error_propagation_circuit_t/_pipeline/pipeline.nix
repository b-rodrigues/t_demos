...
EOF
      ...
      good_val = (100 / 5)
EOF
      echo "      res1 = serialize(good_val, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(good_val))" >> node_script.t
  ...
  recovered_with_match = stdenv.mkDerivation {
    name = "recovered_with_match";
    buildInputs = [ tBin  ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
...
EOF
      ...
      cat <<'EOF' >> node_script.t
      recovered_with_match = { risky = (42 / 0); match(risky) { Error { msg } => 0, default => risky } }
EOF
      echo "      res1 = serialize(recovered_with_match, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(recovered_with_match))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  recovered_with_maybe_pipe = stdenv.mkDerivation {
    name = "recovered_with_maybe_pipe";
    buildInputs = [ tBin  ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
...
EOF
      ...
      cat <<'EOF' >> node_script.t
      recovered_with_maybe_pipe = { risky = (99 / 0); (risky ?|> \(x) if (is_error(x)) { -1 } else { x }) }
EOF
      echo "      res1 = serialize(recovered_with_maybe_pipe, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(recovered_with_maybe_pipe))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  short_circuit_then_recover = stdenv.mkDerivation {
    name = "short_circuit_then_recover";
    buildInputs = [ tBin  ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
...
EOF
      ...
      cat <<'EOF' >> node_script.t
      short_circuit_then_recover = { risky = (1 / 0); piped = (risky |> \(x) (x + 100)); (piped ?|> \(x) if (is_error(x)) { 0 } else { x }) }
EOF
      echo "      res1 = serialize(short_circuit_then_recover, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(short_circuit_then_recover))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin good_val recovered_with_match recovered_with_maybe_pipe short_circuit_then_recover ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${good_val} $out/good_val
      cp -r ${recovered_with_match} $out/recovered_with_match
      cp -r ${recovered_with_maybe_pipe} $out/recovered_with_maybe_pipe
      cp -r ${short_circuit_then_recover} $out/short_circuit_then_recover
    '';
  };
}
