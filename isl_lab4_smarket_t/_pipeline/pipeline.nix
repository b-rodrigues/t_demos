...
rec {
  ...
  smarket_raw = stdenv.mkDerivation {
    name = "smarket_raw";
...
EOF
      ...
      echo "      smarket_raw = {" >> node_script.t
      cat <<'EOF' >> node_script.t
read_csv("data/Smarket.csv")
EOF
      echo "      }" >> node_script.t
      echo "      res1 = write_arrow(smarket_raw, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(smarket_raw))" >> node_script.t
  ...
  data_node = stdenv.mkDerivation {
    name = "data_node";
    buildInputs = [ tBin r-env smarket_raw ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_smarket_raw = smarket_raw;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_smarket_raw=${smarket_raw}
...
EOF
      ...
      echo "if (file.exists(file.path(\"$T_NODE_smarket_raw\", \"class\")) && readLines(file.path(\"$T_NODE_smarket_raw\", \"class\"), 1) == \"VError\") {" >> node_script.R
      echo "  smarket_raw <- r_read_json(file.path(\"$T_NODE_smarket_raw\", \"artifact\"))" >> node_script.R
      echo "} else {" >> node_script.R
      echo "  smarket_raw <- r_read_arrow(file.path(\"$T_NODE_smarket_raw\", \"artifact\"))" >> node_script.R
      echo "}" >> node_script.R
...
EOF
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin smarket_raw data_node r_model py_model ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${smarket_raw} $out/smarket_raw
