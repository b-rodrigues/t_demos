...
EOF
      ...
      echo "        config_py = __node_runner()" >> node_script.py
      ...
      echo "if py_is_error(config_py):" >> node_script.py
      echo "    py_write_error(config_py, os.path.join(os.environ['out'], 'artifact'))" >> node_script.py
      echo "else:" >> node_script.py
      cat <<'EOF' >> node_script.py
    (lambda obj, path: __import__('yaml').dump(obj, open(path, 'w'), default_flow_style=False))(config_py, os.path.join(os.environ['out'], 'artifact'))
EOF
      echo "    with open(os.path.join(os.environ['out'], 'class'), 'w') as f: f.write(py_visual_class(config_py))" >> node_script.py
  ...
  config_r = stdenv.mkDerivation {
    name = "config_r";
    buildInputs = [ tBin r-env config_py ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_config_py = config_py;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_config_py=${config_py}
...
EOF
      ...
      echo "if (file.exists(file.path(\"$T_NODE_config_py\", \"class\")) && readLines(file.path(\"$T_NODE_config_py\", \"class\"), 1) == \"VError\") {" >> node_script.R
      echo "  config_py <- r_read_json(file.path(\"$T_NODE_config_py\", \"artifact\"))" >> node_script.R
      echo "} else {" >> node_script.R
      echo "  config_py <- (function(path) yaml::read_yaml(path))(file.path(\"$T_NODE_config_py\", \"artifact\"))" >> node_script.R
      echo "}" >> node_script.R

      echo "captured_warns <- list()" >> node_script.R
      echo "config_r <- withCallingHandlers({" >> node_script.R
...
EOF
      ...
      echo "})" >> node_script.R
       echo "if (r_is_error(config_r)) {" >> node_script.R
       echo "  r_write_error(config_r, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_save_viz_metadata(config_r, file.path(Sys.getenv('out'), 'viz'))
  saveRDS(config_r, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(config_r), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin config_py config_r ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${config_py} $out/config_py
      cp -r ${config_r} $out/config_r
    '';
  };
}
