...
EOF
      ...
      cat <<'EOF' >> node_script.t
      base_data = [owner: "antigravity", version: "0.51.2", retry: 3, tags: ["data", "science", "reproducible"]]
EOF
      echo "      res1 = serialize(base_data, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(base_data))" >> node_script.t
  ...
  updated_version = stdenv.mkDerivation {
    name = "updated_version";
    buildInputs = [ tBin base_data ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_base_data = base_data;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_base_data=${base_data}
...
EOF
      ...
      echo "base_data = deserialize(\"$T_NODE_base_data/artifact\")" >> node_script.t

      cat <<'EOF' >> node_script.t
      updated_version = { l = col_lens("version"); over(base_data, l, \(v) str_join([v, "-released"], sep = "")) }
EOF
      echo "      res1 = serialize(updated_version, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(updated_version))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  updated_retry = stdenv.mkDerivation {
    name = "updated_retry";
    buildInputs = [ tBin updated_version ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_updated_version = updated_version;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_updated_version=${updated_version}
...
EOF
      ...
      echo "updated_version = deserialize(\"$T_NODE_updated_version/artifact\")" >> node_script.t

      cat <<'EOF' >> node_script.t
      updated_retry = { l = col_lens("retry"); over(updated_version, l, \(x) (x + 1)) }
EOF
      echo "      res1 = serialize(updated_retry, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(updated_retry))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  both_updated = stdenv.mkDerivation {
    name = "both_updated";
    buildInputs = [ tBin base_data ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
...
EOF
      ...
      cat <<'EOF' >> node_script.t
      both_updated = { vl = col_lens("version"); rl = col_lens("retry"); modify(base_data, vl, \(v) str_join([v, "-released"], sep = ""), rl, \(x) (x + 1)) }
EOF
      echo "      res1 = serialize(both_updated, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(both_updated))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin base_data updated_version updated_retry both_updated ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${base_data} $out/base_data
      cp -r ${updated_version} $out/updated_version
      cp -r ${updated_retry} $out/updated_retry
      cp -r ${both_updated} $out/both_updated
    '';
  };
}
