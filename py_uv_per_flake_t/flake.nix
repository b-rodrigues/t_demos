{
  description = "py_uv_per_flake_t — a T data analysis project";

  inputs = {
    nixpkgs.url = "github:rstats-on-nix/nixpkgs/2026-06-22";
    flake-utils.url = "github:numtide/flake-utils";
    t-lang.url = "github:b-rodrigues/tlang/v0.54.0";
    # Optional UV/uv2nix Python dependency backend
  ...
  outputs = { self, nixpkgs, flake-utils, t-lang, pyproject-nix, uv2nix, pyproject-build-systems }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Python environment
        pyWorkspace = uv2nix.lib.workspace.loadWorkspace { workspaceRoot = ./. + "/python"; };
        pyOverlay = pyWorkspace.mkPyprojectOverlay { sourcePreference = "wheel"; };
        pySet = (pkgs.callPackage pyproject-nix.build.packages { python = pkgs.python312; }).overrideScope (pkgs.lib.composeManyExtensions [ pyOverlay pyproject-build-systems.overlays.default ]);
        py-env = pySet.mkVirtualEnv "t-python-uv-env" pyWorkspace.deps.default;
...
EOF
            ...
            echo "=================================================="
            echo "T Project: py_uv_per_flake_t"
            echo "=================================================="
            echo ""
