...
EOF
      ...
      cat <<'EOF' >> node_script.t
      data_node = (read_csv("data/mtcars.csv", separator = "|") |> mutate(cyl = to_factor($cyl), am = to_factor($am)))
EOF
      echo "      res1 = write_csv(data_node, \"$out/artifact\")" >> node_script.t
  ...
  model_r_node = stdenv.mkDerivation {
    name = "model_r_node";
...
EOF
      ...
      echo "  data_node <- r_read_csv(file.path(\"$T_NODE_data_node\", \"artifact\"))" >> node_script.R
      echo "}" >> node_script.R

      echo "captured_warns <- list()" >> node_script.R
      echo "model_r_node <- withCallingHandlers({" >> node_script.R
...
EOF
      ...
      echo "})" >> node_script.R
       echo "if (r_is_error(model_r_node)) {" >> node_script.R
       echo "  r_write_error(model_r_node, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_pmml(model_r_node, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(model_r_node), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
...
EOF
      ...
      echo "captured_warns <- list()" >> node_script.R
      echo "rf_r_node <- withCallingHandlers({" >> node_script.R
      echo "  local({" >> node_script.R
      echo "    tryCatch({" >> node_script.R
      cat <<'EOF' >> node_script.R
...
EOF
      ...
      echo "  invokeRestart('muffleWarning')" >> node_script.R
      echo "})" >> node_script.R
       echo "if (r_is_error(rf_r_node)) {" >> node_script.R
       echo "  r_write_error(rf_r_node, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_pmml(rf_r_node, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(rf_r_node), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
       echo "  r_write_warnings(captured_warns, file.path(Sys.getenv('out'), 'warnings'))" >> node_script.R
       echo "}" >> node_script.R
      mkdir -p $out
      Rscript node_script.R
  ...
  model_py_node = stdenv.mkDerivation {
    name = "model_py_node";
...
EOF
      ...
      echo "    data_node = py_read_csv(os.path.join(\"$T_NODE_data_node\", \"artifact\"))" >> node_script.py
...
EOF
      ...
      echo "        model_py_node = __node_runner()" >> node_script.py
      ...
      echo "if py_is_error(model_py_node):" >> node_script.py
      echo "    py_write_error(model_py_node, os.path.join(os.environ['out'], 'artifact'))" >> node_script.py
      echo "else:" >> node_script.py
      cat <<'EOF' >> node_script.py
    py_write_pmml(model_py_node, os.path.join(os.environ['out'], 'artifact'))
EOF
      echo "    with open(os.path.join(os.environ['out'], 'class'), 'w') as f: f.write(py_visual_class(model_py_node))" >> node_script.py
  ...
  verify_node = stdenv.mkDerivation {
    name = "verify_node";
    buildInputs = [ tBin data_node model_py_node model_r_node rf_r_node ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;
    ...
    T_NODE_data_node = data_node;
    T_NODE_model_py_node = model_py_node;
    T_NODE_model_r_node = model_r_node;
    T_NODE_rf_r_node = rf_r_node;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_data_node=${data_node}
      export T_NODE_model_py_node=${model_py_node}
      export T_NODE_model_r_node=${model_r_node}
      export T_NODE_rf_r_node=${rf_r_node}
...
EOF
      ...
      echo "data_node = read_csv(\"$T_NODE_data_node/artifact\")" >> node_script.t
      echo "model_py_node = t_read_pmml(\"$T_NODE_model_py_node/artifact\")" >> node_script.t
      echo "model_r_node = t_read_pmml(\"$T_NODE_model_r_node/artifact\")" >> node_script.t
      echo "rf_r_node = t_read_pmml(\"$T_NODE_rf_r_node/artifact\")" >> node_script.t

      echo "      verify_node = {" >> node_script.t
      cat <<'EOF' >> node_script.t
...
EOF
      echo "      }" >> node_script.t
      echo "      res1 = serialize(verify_node, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(verify_node))" >> node_script.t
      echo "      if (is_error(res2)) { print(\"Class write failed:\"); print(res2); exit(1) } else { 0 }" >> node_script.t
      mkdir -p $out
      t run --unsafe --mode repl node_script.t
    '';
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin data_node model_r_node rf_r_node model_py_node verify_node ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${data_node} $out/data_node
      cp -r ${model_r_node} $out/model_r_node
      cp -r ${rf_r_node} $out/rf_r_node
      cp -r ${model_py_node} $out/model_py_node
