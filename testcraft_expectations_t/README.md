...
- **Data Quality & Structure Assertions**: `expect_nrow`, `expect_ncol`, `expect_colnames`, `expect_has_colnames`, `expect_unique`, `expect_fields`, `expect_in`, `expect_length`, `expect_no_na`, `expect_set_equal`, `expect_empty`
...
- **Condition & Error Verification**: `expect_error`, `expect_warning` (with `kind` and `message` filters)
