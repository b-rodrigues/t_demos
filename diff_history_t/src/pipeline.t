...
-- Build completion assertions (produce final expression so failures are visible)
build_ok = !is_error(res1) && !is_error(res2) && !is_error(res3)
assert(build_ok, "All 3 pipeline versions should build successfully")
assert(nrow(hist) >= 1, "build_log_history should have at least 1 entry")
h1 = build_log_history(p1)
h2 = build_log_history(p2)
all_entries = nrow(h1) + nrow(h2) + nrow(hist)
assert(all_entries >= 3, "all 3 pipeline versions should have at least 3 build log entries")
