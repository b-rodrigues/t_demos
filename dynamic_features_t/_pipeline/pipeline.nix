...
EOF
  ...
  features_t = stdenv.mkDerivation {
    name = "features_t";
...
EOF
      ...
      echo 'import stats' >> node_script.t
      echo 'import "src/features.t"' >> node_script.t


      echo "raw_data = read_arrow(\"$T_NODE_raw_data/artifact\")" >> node_script.t

      echo "      features_t = {" >> node_script.t
      cat <<'EOF' >> node_script.t
...
EOF
      echo "      }" >> node_script.t
      echo "      res1 = write_arrow(features_t, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(features_t))" >> node_script.t
  ...
  features_r = stdenv.mkDerivation {
    name = "features_r";
...
EOF
      ...
      echo "captured_warns <- list()" >> node_script.R
      echo "features_r <- withCallingHandlers({" >> node_script.R
      echo "  local({" >> node_script.R
      echo "    tryCatch({" >> node_script.R
      cat <<'EOF' >> node_script.R
        raw_data %>%
...
EOF
      ...
      echo "  invokeRestart('muffleWarning')" >> node_script.R
      echo "})" >> node_script.R
       echo "if (r_is_error(features_r)) {" >> node_script.R
       echo "  r_write_error(features_r, file.path(Sys.getenv('out'), 'artifact'))" >> node_script.R
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_arrow(features_r, file.path(Sys.getenv('out'), 'artifact'))
EOF
       echo "  writeLines(r_visual_class(features_r), file.path(Sys.getenv('out'), 'class'))" >> node_script.R
       echo "  r_write_warnings(captured_warns, file.path(Sys.getenv('out'), 'warnings'))" >> node_script.R
       echo "}" >> node_script.R
      mkdir -p $out
      Rscript node_script.R
  ...
  validation = stdenv.mkDerivation {
    name = "validation";
    buildInputs = [ tBin features_r features_t ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_features_r = features_r;
    T_NODE_features_t = features_t;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_features_r=${features_r}
      export T_NODE_features_t=${features_t}
...
EOF
      ...
      echo "features_r = read_arrow(\"$T_NODE_features_r/artifact\")" >> node_script.t
      echo "features_t = read_arrow(\"$T_NODE_features_t/artifact\")" >> node_script.t
...
EOF
      echo "      }" >> node_script.t
      echo "      res1 = serialize(validation, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin raw_data features_t features_r validation ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${raw_data} $out/raw_data
      cp -r ${features_t} $out/features_t
      cp -r ${features_r} $out/features_r
