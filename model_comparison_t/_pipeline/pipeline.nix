...
EOF
      ...
      echo "})" >> node_script.R
       ...
       echo "} else {" >> node_script.R
       cat <<'EOF' >> node_script.R
  r_write_json(r_model, file.path(Sys.getenv('out'), 'artifact'))
EOF
  ...
  py_model = stdenv.mkDerivation {
    name = "py_model";
    buildInputs = [ tBin py-env raw_data ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;
...
EOF
      ...
      echo "if os.path.exists(os.path.join(\"$T_NODE_raw_data\", \"class\")) and open(os.path.join(\"$T_NODE_raw_data\", \"class\")).read().strip() == \"VError\":" >> node_script.py
      echo "    raw_data = py_read_json(os.path.join(\"$T_NODE_raw_data\", \"artifact\"))" >> node_script.py
      echo "else:" >> node_script.py
      echo "    raw_data = py_read_arrow(os.path.join(\"$T_NODE_raw_data\", \"artifact\"))" >> node_script.py

      echo "def __node_runner():" >> node_script.py
      echo '    global raw_data
' >> node_script.py
      cat <<'EOF' >> node_script.py
    X = raw_data[['hp', 'wt']]
    y = raw_data['mpg']
...
EOF
      ...
      echo "        py_model = __node_runner()" >> node_script.py
      ...
      echo "if py_is_error(py_model):" >> node_script.py
      echo "    py_write_error(py_model, os.path.join(os.environ['out'], 'artifact'))" >> node_script.py
      echo "else:" >> node_script.py
      cat <<'EOF' >> node_script.py
    py_write_json(py_model, os.path.join(os.environ['out'], 'artifact'))
EOF
      echo "    with open(os.path.join(os.environ['out'], 'class'), 'w') as f: f.write(py_visual_class(py_model))" >> node_script.py
  ...
  comparison_result = stdenv.mkDerivation {
    name = "comparison_result";
    buildInputs = [ tBin py_model r_model ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;
...
EOF
      ...
      echo "py_model = t_read_json(\"$T_NODE_py_model/artifact\")" >> node_script.t
      echo "r_model = t_read_json(\"$T_NODE_r_model/artifact\")" >> node_script.t

      echo "      comparison_result = {" >> node_script.t
      cat <<'EOF' >> node_script.t
res = [
        r_r2: r_model, 
        py_r2: py_model, 
...
EOF
      echo "      }" >> node_script.t
      echo "      res1 = t_write_json(comparison_result, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(comparison_result))" >> node_script.t
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin raw_data r_model py_model comparison_result ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${raw_data} $out/raw_data
      cp -r ${r_model} $out/r_model
      cp -r ${py_model} $out/py_model
      cp -r ${comparison_result} $out/comparison_result
    '';
  };
}
