...
rec {
  ...
  boston_raw = stdenv.mkDerivation {
    name = "boston_raw";
...
EOF
      ...
      echo "      boston_raw = {" >> node_script.t
      cat <<'EOF' >> node_script.t
read_csv("data/Boston.csv")
EOF
      echo "      }" >> node_script.t
      echo "      res1 = write_arrow(boston_raw, \"$out/artifact\")" >> node_script.t
      echo "      if (is_error(res1)) { print(\"Serialization failed:\"); print(res1); exit(1) } else { 0 }" >> node_script.t
      echo "      res2 = write_text(\"$out/class\", type(boston_raw))" >> node_script.t
  ...
  data_node = stdenv.mkDerivation {
    name = "data_node";
    buildInputs = [ tBin r-env boston_raw ] ++ globalBuildInputs;
    T_JPMML_STATSMODELS_JAR = if (pkgs ? jpmml-statsmodels) then "${pkgs.jpmml-statsmodels}/share/java/jpmml-statsmodels.jar" else "";
    T_JPMML_EVALUATOR_JAR = if (pkgs ? jpmml-evaluator) then "${pkgs.jpmml-evaluator}/share/java/jpmml-evaluator.jar" else "";
    MPLCONFIGDIR = ".";
    src = sources;

    T_NODE_boston_raw = boston_raw;
    buildCommand = ''
      cp -r $src/* . || true
      chmod -R u+w .
      export T_NODE_boston_raw=${boston_raw}
...
EOF
      ...
      echo "if (file.exists(file.path(\"$T_NODE_boston_raw\", \"class\")) && readLines(file.path(\"$T_NODE_boston_raw\", \"class\"), 1) == \"VError\") {" >> node_script.R
      echo "  boston_raw <- r_read_json(file.path(\"$T_NODE_boston_raw\", \"artifact\"))" >> node_script.R
      echo "} else {" >> node_script.R
      echo "  boston_raw <- r_read_arrow(file.path(\"$T_NODE_boston_raw\", \"artifact\"))" >> node_script.R
      echo "}" >> node_script.R
...
EOF
      ...
      echo "else:" >> node_script.py
      cat <<'EOF' >> node_script.py
    py_write_pmml(py_model, os.path.join(os.environ['out'], 'artifact'))
EOF
  ...
  pipeline_output = stdenv.mkDerivation {
    ...
    buildInputs = [ tBin boston_raw data_node r_model py_model ] ++ globalBuildInputs;
    buildCommand = ''
      mkdir -p $out
      cp -r ${boston_raw} $out/boston_raw
